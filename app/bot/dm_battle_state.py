...
class DmBattleState:
    ...
    # 시트 기록용 영속 키. [전투 발생] 응답 게시물의 status_id로 한 번
    # 고정되며, active_post_id와 달리 페이즈마다 바뀌지 않는다.
    field_id: str

    # 현재 스레드 tip. 답글 매칭과 다음 게시물의 in_reply_to_id로 쓰인다.
    active_post_id: int

    # 최초 [전투 발생] DM의 visibility. 세션 내내 고정해 봇이 새로 올리는
    # 게시물이 이 값을 따르게 한다(개별 답글은 원본 메시지를 따르므로 무관).
    visibility: str = "direct"

    # "direct" 스레드는 게시물마다 명시적으로 멘션된 계정만 볼 수 있어,
    # 봇이 올리는 모든 게시물에 이 목록을 멘션으로 붙여야 한다.
