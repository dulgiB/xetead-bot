...
"필드" 시트는 전투 세션(본 전투/대련/결투/상시전투)의 진행 상태 스냅샷이자
...
class FieldBattleType(str, Enum):
    ...
    재개(재기동 복원) 시 이 값으로 본 전투/대련/결투/상시전투/상시조사 중
...
def upsert_field_row(
    ...
    - 못 찾았고 battle_type이 동시 1개 슬롯 전제(`_SINGLE_SLOT_BATTLE_TYPES`)면,
      가장 위(가장 최근)의 같은 battle_type 행을 대신 갱신한다. 대련/결투/
      상시전투는 동시에 여러 개 진행될 수 있어 이 fallback을 적용하지 않는다.
