...
정산 등)은 `format_battle_reply_inline()`으로 계산식을 본문에 그대로(↳)
포함해 기존처럼 표시한다.
"""
...
def format_battle_reply_inline(
    ...
    그대로 포함한 단일 문자열을 반환한다."""
    blocks = [
        _format_part_inline(context, caster_id, part_result)
        for part_result in part_results
    ]
    return "\n\n".join(blocks)
...
def _format_part_inline(
    ...
    _part, header, log_entries = _header_and_log_entries(caster_id, part_result)
    body_lines = [_format_entry_inline(context, entry) for entry in log_entries]
    if not body_lines:
        return header
    return header + "\n" + "\n".join(body_lines)
