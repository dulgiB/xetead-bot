...
class BuffContainer:
    ...
    def on_round_end(self) -> tuple[list[BattleLogEntry], list[BuffUid]]:
        ...
        차감은 부여 시점과 무관하게 필드의 모든 버프에 똑같이 적용된다 —
        대련/결투/상시전투도 본 전투와 같은 규칙을 쓴다(`PracticeRoundManager`
        참고). 그래서 이 구조에서 라운드의 마지막 행동 차례에 걸린 1턴짜리
        효과는 상대에게 행동 기회를 주지 못한 채 사라지므로, 상대가 그 상태로
        행동하기를 기대하는 효과는 2턴 이상으로 적는다.
        """
