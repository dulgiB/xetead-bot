...
def process_ally_command(
    ...
    character_action_events = [buff.create_event() for buff in character_action_buffs]
    ban_action_events = [
    ...
    ban_event_results = [
        ...
        for event in ban_action_events
    ]
    ...
    for damage_data in command.damage_list:
        ...
        attacker_events = [buff.create_event() for buff in attacker_buff_list]

        attack_target_buff_list = context.buff_container.get_buffs_by(
            damage_data.target_id, BuffApplyTiming.ON_HIT
        )
        target_events = [buff.create_event() for buff in attack_target_buff_list]
    ...
    for heal_data in command.heal_list:
        ...
        healer_modifiers = [buff.create_event() for buff in healer_buff_list]

        heal_target_buff_list = context.buff_container.get_buffs_by(
            heal_data.target_id, BuffApplyTiming.ON_RECEIVE_HEAL
        )
        heal_target_modifiers = [buff.create_event() for buff in heal_target_buff_list]
