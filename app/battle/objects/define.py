...
class BuffType(str, Enum):
    """버프의 분류. "버프" 시트의 `type` 컬럼에서 온다.

    "이로운가 해로운가"가 아니라 **다른 효과가 이 버프를 집계 대상으로 삼는가**를
    정하는 값이다. 그래서 서사상 명백히 해로운 효과여도 NEUTRAL일 수 있다 —
    예컨대 [도발]은 대상에게 불리하지만, "디버프가 걸린 적에게 주는 대미지 증가"
    계열이 노리는 것은 아군이 깔아 둔 디버프와의 시너지이지 도발 자체가 아니다.
    도발까지 세면 탱커가 도발을 거는 것만으로 딜러의 증폭이 켜져, 그 증폭이
    의도한 콤보와 무관하게 상시 발동한다.

    NEUTRAL은 BUFF/DEBUFF 어느 집계에도 잡히지 않고 디버프 해제로도 지워지지
    않는다(해제 불가 마커 등).
    """
