...
def test_attack_command_separates_damage_and_calculation():
    ...
    dmg = 100 - target.status.curr_hp
    assert lines[1] == f"▹ 적군 1 | -{dmg} → {target.status.curr_hp}/100"
    assert len(lines) == 2
    assert "↳" not in reply
    ...
    assert calc_lines[1].endswith(f"→ 「-{dmg}」")


def test_fixed_damage_skill_omits_calculation_line():
    """FIXED 값은 modifier가 전혀 없으면 계산식을 생략한다."""
    skill = SkillData(
        id="강타",
...
def test_enemy_skill_preview_shows_description_when_revealed():
    ...
    reply, _calc = format_battle_reply(ctx, caster_id, new_results, show_skill_preview=True)
