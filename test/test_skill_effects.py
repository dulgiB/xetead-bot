...
def test_damage_skill_costs_2(damage_skill_setup):
    ...
    cmd = parse_character_command(user_id, "[강타/적군 1]", ctx)
    manager.process_command(cmd)
...
def test_vampiric_skill_reduces_target_hp(vampiric_skill_setup):
    ...
    cmd = parse_character_command(CharacterId("아군 1"), "[흡혈/적군 1]", ctx)
    manager.process_command(cmd)
...
def test_vampiric_skill_heals_self_for_half_damage(vampiric_skill_setup):
    ...
    cmd = parse_character_command(user_id, "[흡혈/적군 1]", ctx)
    manager.process_command(cmd)
