...
## 테스트에 실제 캠페인 데이터 노출 금지

테스트 코드(특히 실제 스프레드시트 행을 `from_dict()`로 그대로 재현하는 통합
테스트)를 작성할 때, 실제 캠페인의 캐릭터 이름·스킬명·패시브명 등 고유명사를
문자열 그대로 코드에 박아넣지 않는다.

- **캐릭터 이름**: 일반화된 placeholder(예: `Catastrophe`)를 쓴다.
- **스킬/패시브 id**: 기능을 드러내는 일반 이름(예: `Cost2Skill`, `Cost3Skill`,
  `PassiveSkill`, `PassiveBuff`)으로 대체한다. 스킬 설명(`description`) 등
  메커니즘을 서술하는 텍스트는 그대로 유지해도 된다 — 문제는 실제 캐릭터를
  특정할 수 있는 고유명사이지, 한글 텍스트 자체가 아니다.
- 버프처럼 여러 캐릭터가 공유하는 범용 게임 시스템 명칭(`재앙`/`BuffCatastrophe`,
  `도발`/`BuffTaunt` 등)은 특정 캐릭터를 특정하지 않으므로 예외로 둔다.
- 이미 커밋된 실제 데이터를 발견하면, 히스토리에 흔적이 남지 않도록 스크럽한
  뒤 force-push한다. GPG 서명된 병합 커밋이 히스토리에 있으면 `git filter-repo`
  (심지어 `--refs`로 범위를 좁혀도)가 그 서명을 벗겨내며 이후 모든 커밋의
  해시가 cascade로 바뀌어 `main`과의 공유 히스토리가 깨진다 — 대신 안전한
  공유 지점(main과의 병합 베이스)에서 `git cherry-pick`으로 커밋을 그대로
  재현한 뒤, 노출된 커밋만 `git commit --amend`로 내용/메시지를 고쳐 안전하게
  스크럽한다. 이미 `main`에 병합되어 공유 중인 커밋 자체는 재작성 범위에서
  제외하고 별도로 상의한다.
