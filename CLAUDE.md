...
**스킬/버프 설명에 "고정"이라고 적은 대미지는 주는/받는 대미지 버프의
배율을 받지 않는다.** 값 소스가 `FIXED`면 자동으로 그렇게 되고, 계수는
살린 채 배율만 떼야 하는 항목(스택 수 × 계수 등)은
`BaseValueIndicator.ignores_value_modifiers`를 켠다
(`make_coefficient_damage_calc(ignores_value_modifiers=True)`). 한쪽만
빠뜨리면 같은 설명을 읽은 플레이어가 버프 하나로 수치가 달라지는 것을
설명할 수 없다.

제외 대상은 버프뿐이다 — 부활 페널티·대리 수령 경감·키워드 보정처럼
`applies_to_fixed=True`인 게임 메커니즘은 고정 대미지에도 그대로 걸린다.
...
| 아이템 | ✗ | ✗ | ✗ |
| 키워드 보정 | ✗ | ✓ (실제 체력 소모) | ✗ |
| 패배 대가 | 없음 | 실제 체력 −`max_hp // 2` | 없음 |

### 승부가 난 라운드는 닫지 않는다

`_finalize_practice_phase()`는 어느 페이즈에서든 한쪽 팀의 체력 합이 0이
되면 `ps.end_round()`를 **건너뛰고** 바로 종료로 간다. 전투 종료 처리
(`BuffBase.on_battle_end()`)도 양 팀이 모두 살아서 끝났을 때만 돈다. 닫아
버리면 이긴 쪽이 자기에게 걸린 DoT나 남은 대가로 함께 쓰러져 무승부가 되고,
그러면 진 쪽마저 패배 대가를 치르지 않는다.

정상적으로 닫힌 라운드의 종료 처리는 `PracticeRoundManager`의
`get_last_round_end_log_entries()`/`get_last_eliminated_characters()`로 꺼내
다음 라운드 공지(또는 라운드 상한으로 끝났다면 종료 게시물)에 싣는다 —
대련 계열 게시물에는 계산식을 접어 둘 CW 후속 게시물이 없으므로 전투 종료
정산과 같이 계산식까지 본문에 담는다. 싣지 않으면 플레이어는 라운드가
넘어갈 때 체력이 줄어든 것만 보고 이유를 알 수 없다.

### 포지션 선언 단계부터 "필드" 시트에 남는다

`_persist_practice_prep()`이 준비 게시물이 올라간 직후와 포지션 선언이
들어올 때마다 행을 갱신한다(`round_n=0`, 캐릭터 스냅샷은 빈 목록).
복원에 필요한 참여 대상과 지금까지의 선언은 메타의 `expected_accts`/
`positions`에 싣는다 — 이 단계의 행에는 배치된 캐릭터가 하나도 없어 그
둘이 유일한 복원 근거다. `field_restore`는 `round_n < 1`인 행을 선언 단계로
보고 `_restore_practice_prep()`으로 보낸다.

`field_id`는 준비 게시물 id로 이때 고정되므로, 라운드가 열린 뒤에도
`_begin_practice_rounds()`가 같은 값을 써서 같은 행이 이어진다.
...
- **패배 대가**: `main.py`의 `_apply_duel_defeat_penalty()`가 전투 종료 시점에
  ...
  깎는 양은 고정값이 아니라 **캐릭터별 최대 체력의
  `DUEL_DEFEAT_HP_PENALTY_PERCENT`%**(내림)이므로 `max_hp`도 같은 함수가 시트에서
  읽는다 — 명부에는 있지만 필드에서 이미 빠진 캐릭터의 `max_hp`가 라이브
  상태(`persistent_hp`)에는 없을 수 있어 호출측에서 계산할 수 없다. `max_hp`를
  읽지 못한 캐릭터는 0을 적용하지 않고 실패로 돌려 종료 게시물이 admin에게
  확인을 요청한다.
...
위치·진영·잔여 코스트·체력과 함께 **버프 상태**가 들어간다
(`log_sheets.build_field_buffs()`) — 남은 턴/횟수·스택·부여 일련번호까지
담아야 복원이 "다시 부여"가 아니라 "그 상태 그대로 이어받기"가 된다. 버프는
어느 시트에도 진실 공급원이 없어, 여기 남기지 않으면 재기동 때마다 통째로
사라진다.

- **복원은 `on_battle_start()` 뒤에 한다**(`_restore_buffs()`). 앞에 두면
  "전투 시작" 트리거가 같은 버프를 다시 걸며 지속시간을 시트 값으로 리셋해,
  남은 턴이 스냅샷보다 늘어난다. 이미 같은 uid가 있으면 새로 만들지 않고
  `BuffContainer.restore()`가 그 인스턴스를 스냅샷 값으로 맞춘다
  (`add()`는 "지금 새로 부여"라 지속시간을 리셋하므로 쓸 수 없다).
- 패시브 스킬 래퍼는 담지 않는다 — "버프" 시트에 없고 `on_battle_start()`이
  다시 등록한다. 시트에서 사라진 id, 필드에 없는 대상도 그 항목만 건너뛴다.
- 행을 커맨드마다 갱신할 전투 종류는 `_PRACTICE_FIELD_TYPES`가 모드 매핑에서
  파생한다. 따로 나열하면 모드를 늘렸을 때 거기만 빠지고, 그 모드는 스냅샷이
  전투 시작 시점에 멈춘 채 복원된다.
